...
%  We have data index range from 41-70.
...
%
%  E.G. id = 41 means experiment 41.
id  = 41;
d_name = strcat('Result\CI', num2str(id),'(var_fixed).mat');
load(d_name)
...
for i = 1:100
   hl_indi = get_indi(hl.hist(1:11,i),Info.Conc(end));
   hl_p    = [hl_p, hl_indi(1)];
   hl_GR1  = [hl_GR1, hl_indi(4)];
   hl_GR2  = [hl_GR2, hl_indi(5)];
   dyn_indi = get_indi(dyn.hist(1:12,i),Info.Conc(end));
   dyn_p    = [dyn_p, dyn_indi(1)];
   dyn_GR1  = [dyn_GR1, dyn_indi(4)];
   dyn_GR2  = [dyn_GR2, dyn_indi(5)];
   sto_indi = get_indi(sto.hist(1:12,i),Info.Conc(end));
...
hl_p   = [mean(hl.p), 1 - mean(hl.p)];
dyn_p  = [mean(dyn.p), 1 - mean(dyn.p)];
sto_p  = [mean(sto.p), 1 - mean(sto.p)];
