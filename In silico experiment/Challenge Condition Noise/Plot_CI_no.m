...
%  We have data index range from 91-121 for high observation noise.
...
%  different observation noise. In particular,
%  91-110: observation noise std in [100,300,500]
%  111-121: observation noise std in [100,200,300,400,500]
%  Please use 'id' and 'no' to looking for correspondent experiment. 
%
%  E.G. id = 91, no = 3 means experiment 91 with observation noise std 500.

id  = 91;
no  = 3;
d_name = strcat('Result\sup_Noise_quant', num2str(id),'(var_fixed).mat');
load(d_name)
...
for i = 1:100
   hl_indi = get_indi(hl.hist(11*no-10:11*no,i),Info.Conc(end));
   hl_p    = [hl_p, hl_indi(1)];
   hl_GR1  = [hl_GR1, hl_indi(4)];
   hl_GR2  = [hl_GR2, hl_indi(5)];
   dyn_indi = get_indi(dyn.hist(12*no-11:12*no,i),Info.Conc(end));
   dyn_p    = [dyn_p, dyn_indi(1)];
   dyn_GR1  = [dyn_GR1, dyn_indi(4)];
   dyn_GR2  = [dyn_GR2, dyn_indi(5)];
   sto_indi = get_indi(sto.hist(12*no-11:12*no,i),Info.Conc(end));
...
hl_p   = [mean(hl.p(no,:)), 1 - mean(hl.p(no,:))];
dyn_p  = [mean(dyn.p(no,:)), 1 - mean(dyn.p(no,:))];
sto_p  = [mean(sto.p(no,:)), 1 - mean(sto.p(no,:))];
